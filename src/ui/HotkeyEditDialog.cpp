...
void HotkeyEditDialog::setupUI()
{
    ...
    m_actionCombo->addItem("复制图片路径 (clipboard_image_path)", "clipboard_image_path");
    m_actionCombo->addItem("去除换行符 (strip_newlines)", "strip_newlines");
    formLayout->addRow("动作:", m_actionCombo);

    m_keySequenceEdit = new QKeySequenceEdit(this);
    m_keySequenceEdit->setMinimumHeight(28);
    QLabel *keyHint = new QLabel("点击下面的输入框，然后按下快捷键", this);
    keyHint->setStyleSheet("color: gray; font-size: 11px;");
    formLayout->addRow("", keyHint);
    formLayout->addRow("快捷键:", m_keySequenceEdit);
    ...
    m_autoSubmitCheck->setToolTip("自动粘贴后模拟回车键提交（慎用）");
...
void HotkeyEditDialog::validateAndAccept()
{
    ...
    if (keySequence.isEmpty()) {
        QMessageBox::warning(this, "错误", "请输入快捷键！");
    ...
    for (int i = 0; i < m_config.hotkeys.size(); ++i) {
        ...
        if (m_config.hotkeys[i].keySequence == keySequence) {
            QMessageBox::warning(this, "错误", "该快捷键已被使用！");
            m_keySequenceEdit->setFocus();
            return;
        }
    }
